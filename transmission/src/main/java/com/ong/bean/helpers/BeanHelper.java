...
public class BeanHelper {
	...
	public static <T, P> Map<T, P> getInstanceIfNull(Map<T, P> map, Class<?> clazz) {
		...
		/**
		 * 1. check if map is null 1.1 map is not null return map 1.2 map is
		 * null return new Instance Class
		 */
		...
		try {
			...
			logger.error("newInstance the class {} ,Exception is {}", clazz, e1.getMessage());
			try {
				resultMap = (Map<T, P>) defaultMapClazz.newInstance();
			} catch (Exception e2) {
				logger.error("newInstance the defaultMapClazz {} ,Exception is {}", defaultMapClazz, e2.getMessage());
